...
class Flight extends Model
{
    ...
    public function generateSeats()
    {
        ...
        foreach ($classes as $class) {
            ...
            $seatsPerRow = $this->getSeatsPerRow($class->class_type); // Fixed typo in method name
            ...
            for ($row = 1; $row <= $rows; $row++) {
                if (!in_array($row, $existingRows)) {
                    for ($column = 1; $column <= $seatsPerRow; $column++) {
                        ...
                        $seatCode = $this->generateSeatCode($row, $column); // Fixed variable name
            ...
            foreach ($existingSeats as $existingSeat) {
                if ($existingSeat->column > $seatsPerRow || $existingSeat->row > $rows) { // Fixed variable name
    ...
    {
        switch ($classType) { // Added case insensitive comparison
            case 'business':
                return 4;
            case 'economy':
                return 6;
